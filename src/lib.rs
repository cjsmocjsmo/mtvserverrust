...
pub fn process_music_images() {
    ...
    for jpg in mp3_imagesvec {
        ...
        let dims = get_image_dims(&jpg);
        let newdims = normalize_music_image(dims);
        let base_dir = split_base_dir(&jpg);
        let file_name = split_filename(&jpg);
        let extension = split_ext(&jpg);

        let artist_results = image_split_artist(&base_dir);
        println!("this is artist: {}", artist_results);

        let album_results = image_split_album(&base_dir);
        ...
        let imginfo = object! {
            ...
            height: newdims.1,
            idx: image_count,
            fsize: get_file_size(&jpg),
            fullpath: &*jpg,
            b64img: to_base64_str(&jpg, newdims.0, newdims.1),
        };
...
pub fn process_mp3s() {
    ...
    for mp3 in mp3svec {
        ...
        let tags = get_tag_info(&mp3);
        let base_dir = split_base_dir(&mp3);
        let filename_results = split_filename(&mp3);
        let music_artist_results = music_split_artist(&base_dir);
        let music_album_results = music_split_album(&base_dir);
        let duration_results = get_duration(&mp3);
        let artc = check_artist(&music_artist_results, &tags.0);
        let albc = check_album(&music_album_results, &tags.1);
        let sc = check_song(&filename_results);
